                ...
                <div className="videos-container">
                    {store.videos && store.videos.slice(0, visibleVideos).map(video => (
                        <div className="video-card">
                            ...
                            <p className="video-footer">Done by <span className="by-name">{video.channel.handle}</span><button className='favButton' onClick={() => { handle_fav_workout(video.id) }}><i class="fa-solid fa-heart"></i></button></p>
                        </div>
                    ))}
                </div>
            )}
